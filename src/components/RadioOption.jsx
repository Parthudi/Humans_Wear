            ...
            {props.options && props.options.length > 0 && props.options.map((option, i) => (
                <FormControl component="fieldset" key={i} >
                    <RadioGroup
                        ...
                        labelPlacement="top">
                            {props.radioFor === "Address" && <CardContaint key={i} index={i} town={option.town} id={option._id} pincode={option.pincode} state={option.state} address={option.address} city={option.city} mobile={option.mobile} placeType={option.placetype}  modeOfpayment={option.modeofpayment} />}
